...
const firebaseConfig = {
    apiKey: "AIzaSyAX17aJHTql9TUrSc1urYI0iORmbDtIMNA",
    authDomain: "ai-career-advisor-e962b.firebaseapp.com",
    projectId: "ai-career-advisor-e962b",
    storageBucket: "ai-career-advisor-e962b.firebasestorage.app",
    messagingSenderId: "1001156175608",
    appId: "1:1001156175608:web:48106761bd139c0f017d0d",
    measurementId: "G-H6T06YJSZR"
};
...
export async function logCustomEvent(eventName, eventData = {}) {
    try {
...
class UltimateCareerAdvisorApp {
    ...
    simulateAIProcessing() {
        ...
        // Start Gemini analysis in parallel with animation
        let geminiPromise = new Promise((resolve) => {
            this._geminiAnalysisCallback = resolve;
        });
        this._startGeminiAnalysis();
        ...
        const updateProgress = () => {
            ...
            if (progress < 100) {
                ...
                // Wait for Gemini to finish if not done
                geminiPromise.then(() => {
                    loadingDone = true;
    ...
    // Helper to start Gemini analysis and resolve when done
    ...
    generateResults(done) {
        console.log('📊 Generating premium career recommendations using Gemini AI');
        this.careerMatches = [];
        const assessmentData = this.assessmentData;
        const prompt = `You are a career advisor AI. Based on the following assessment data, analyze the user's skills, soft skills, personality, interests, and values, and recommend the top 5 best-fit job roles. For each job, provide:\n- Title\n- Short description\n- Estimated match percentage (60-99)\n- Salary range (in INR LPA)\n- Growth rate (%)\n- Market demand (Low/Medium/High/Very High)\n- Category\n- An emoji icon\n- A color gradient for the card\n\nAssessment Data (JSON):\n${JSON.stringify(assessmentData, null, 2)}\n\nRespond in JSON array format as shown:\n[\n  {\n    "title": "Job Title",\n    "icon": "🧑‍💻",\n    "description": "...",\n    "matchPercentage": 95,\n    "salaryRange": "₹8-45 LPA",\n    "growthRate": 42,\n    "marketDemand": "Very High",\n    "gradient": "linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)",\n    "category": "Technology"\n  }, ...\n]`;
        const self = this;
        import('./gemini.js').then(({ callGemini }) => {
            callGemini(prompt, assessmentData).then(result => {
                try {
    ...
    exportPDF(data) {
        // Enhanced PDF export with premium UI and improved layout
        ...
        doc.roundedRect(30, 110, 535, 680, 12, 12, 'F');
        ...
        doc.text(data.completionDate || '', 110, 170);
        doc.text('Total Time:', 50, 190);
        // Format total time (ms) to hh:mm:ss
        let totalTimeStr = '';
        if (typeof data.totalTime === 'number' && !isNaN(data.totalTime)) {
            ...
            totalTimeStr = `${hours.toString().padStart(2,'0')}:${minutes.toString().padStart(2,'0')}:${seconds.toString().padStart(2,'0')}`;
        }
        doc.text(totalTimeStr, 130, 190);

        // Top Career Match
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(15);
        doc.setTextColor(16,185,129);
        doc.text('Top Career Match', 50, 230);
        if (data.careerMatches && data.careerMatches.length > 0) {
            const top = data.careerMatches[0];
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(13);
            doc.setTextColor(60,28,120);
            doc.text(top.title, 60, 255);
            doc.setFont('helvetica', 'normal');
            doc.setFontSize(12);
            doc.setTextColor(80,80,120);
            doc.text(`Category: ${top.category}`, 60, 275);
            doc.text(`Match: ${top.matchPercentage}%`, 60, 295);
            if (top.description) {
                doc.setFontSize(11);
                const descLines = doc.splitTextToSize(top.description, 480);
                doc.text(descLines, 60, 315);
            }
        }
        ...
        doc.setFontSize(14);
        doc.setTextColor(60,28,120);
        doc.text('All Career Matches', 50, 370);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(11);
        doc.setTextColor(80,80,120);
        let y = 390;
        ...
        if (data.assessmentResults && data.assessmentResults.skills && data.assessmentResults.skills.length > 0) {
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(13);
            doc.setTextColor(60,28,120);
            ...
            doc.setFontSize(11);
            doc.setTextColor(80,80,120);
            const skillsLines = doc.splitTextToSize(data.assessmentResults.skills.join(', '), 480);
            doc.text(skillsLines, 60, y+38);
            y += 60;
        }
        if (data.assessmentResults && data.assessmentResults.recommendations) {
            ...
            doc.setFontSize(11);
            doc.setTextColor(80,80,120);
            const recLines = doc.splitTextToSize(data.assessmentResults.recommendations, 480);
            doc.text(recLines, 60, y+38);
        }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = UltimateCareerAdvisorApp;
}